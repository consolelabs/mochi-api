...
func (i *indexer) GetSoulBoundNFT(collectionAddress string) (*res.IndexerSoulBoundNFTResponse, error) {
	url := fmt.Sprintf("%s/api/v1/nft/%s/soulbound", i.cfg.IndexerServerHost, collectionAddress)
	request, err := http.NewRequest("GET", url, nil)
	if err != nil {
		return nil, err
	}
	...
	if response.StatusCode != http.StatusOK {
		...
		_, err = errBody.ReadFrom(response.Body)
		if err != nil {
			return nil, fmt.Errorf("GetSoulBoundNF - failed to read response: %v", err)
		}

		err = fmt.Errorf("GetSoulBoundNF - failed to filter collection metadata with collectionAddress=%s: %v", collectionAddress, errBody.String())
		return nil, err
	}
	...
	data := &res.IndexerSoulBoundNFTResponse{}
	err = json.Unmarshal(body, data)
	if err != nil {
		return nil, err
	}
