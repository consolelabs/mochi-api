...
func (ch *Chain) TransferOffchain(balance float64, toAcc accounts.Account, amount float64, token model.Token, nonce int, all bool) (*types.Transaction, float64, error) {
	var (
		t   *types.Transaction
		err error
	)
	...
	switch token.IsNative {
	case true:
		t, amount, err = ch.transferOffchain(balance, toAcc, amount, nonce, all)
	default:
		t, amount, err = ch.transferTokenOffchain(balance, toAcc, amount, token, nonce, all)
	}
...
func (ch *Chain) transferOffchain(balance float64, toAcc accounts.Account, amount float64, prevTxNonce int, all bool) (*types.Transaction, float64, error) {
...
func (ch *Chain) transferTokenOffchain(balance float64, toAcc accounts.Account, amount float64, token model.Token, prevTxNonce int, all bool) (*types.Transaction, float64, error) {
	privateKey, err := crypto.HexToECDSA(ch.config.CentralizedWalletPrivateKey)
	if err != nil {
		return nil, 0, err
	}
