...
func (pg *pg) GetOneByActivityName(guildID, activityName string) (*model.GuildConfigActivity, error) {
	...
	return config, pg.db.
		Joins("LEFT JOIN activities ON activities.id = guild_config_activities.activity_id").
		Where("guild_config_activities.guild_id = ? AND lower(activities.name) = lower(?) AND guild_config_activities.active = TRUE", guildID, activityName).
		Preload("Activity").
		First(config).Error
}
