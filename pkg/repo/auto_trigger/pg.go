...
func (pq *pg) CountByGuildUser(guildId, userId string) (int64, error) {
	var count int64
	err := pq.db.Model(&model.AutoTrigger{}).Where("discord_guild_id = ? AND user_discord_id = ?", guildId, userId).Count(&count).Error
	return count, err
}

func (pg *pg) FirstOrCreate(autoTrigger *model.AutoTrigger) error {
	return pg.db.Where("discord_guild_id = ? AND user_discord_id = ?", autoTrigger.DiscordGuildId, autoTrigger.UserDiscordId).FirstOrCreate(autoTrigger).Error
}
...
func (pg *pg) GetAutoTriggers(guildId string) ([]model.AutoTrigger, error) {
	...
	db = db.Where("discord_guild_id = ? AND status = ?", guildId, true)
