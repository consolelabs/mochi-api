...
func (pg *pg) GetPendingSubmission(model *model.VaultSubmission) (submission *model.VaultSubmission, err error) {
	return model, pg.db.Where("vault_id = ? and request_id = ? and submitter_profile_id = ? and status = ?", model.VaultId, model.RequestId, model.SubmitterProfileId, consts.TreasurerSubmissionStatusPending).First(&submission).Error
}

func (pg *pg) UpdatePendingSubmission(model *model.VaultSubmission) (*model.VaultSubmission, error) {
	return model, pg.db.Model(&model).Where("vault_id = ? and request_id = ? and submitter_profile_id = ? and status = ?", model.VaultId, model.RequestId, model.SubmitterProfileId, consts.TreasurerSubmissionStatusPending).Update("status", model.Status).Error
}
