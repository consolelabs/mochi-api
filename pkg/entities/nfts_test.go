...
func TestEntity_GetNFTDetailWithName(t *testing.T) {
	type fields struct {
		repo        *repo.Repo
		store       repo.Store
		log         logger.Logger
	...
	// success
	nftCollection.EXPECT().GetByName("Cyber Rabby #1").Return(validNFTCollectionRabby, nil).AnyTimes()
	mockIndexer.EXPECT().GetNFTDetail("0x7D1070fdbF0eF8752a9627a79b00221b53F231fA", "1").Return(validIndexerResponse, nil).AnyTimes()

	// fail - collection has not been added
	nftCollection.EXPECT().GetByName("Cyber Rabby").Return(nil, errors.New("record not found")).AnyTimes()
	// function does not call indexer if record not found in database
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			got, err := e.GetNFTDetailWithName(tt.args.name, tt.args.tokenID)
			if (err != nil) != tt.wantErr {
				t.Errorf("Entity.GetNFTDetailWithName() error = %v, wantErr %v", err, tt.wantErr)
				return
			}
			if !reflect.DeepEqual(got, tt.want) {
				t.Errorf("Entity.GetNFTDetailWithName() = %v, want %v", got, tt.want)
			}
		})
	}
}
