...
func TestEntity_UpdateGuildStats(t *testing.T) {
	...
	guildStatValid := model.DiscordGuildStat{
		ID:                       util.GetNullUUID("45fad0a6-7f71-48d1-b343-ae51614980ed"),
		GuildID:                  "863278424433229854",
		NrOfMembers:              4,
		NrOfUsers:                1,
	...
	disGuStat.EXPECT().UpsertOne(guildStatValid).Return(nil).AnyTimes()
	disGuStat.EXPECT().UpsertOne(guildStatWrong).Return(errors.New("error: invalid guild id")).AnyTimes()
...
func TestEntity_GetByGuildID(t *testing.T) {
	type fields struct {
		repo     *repo.Repo
		store    repo.Store
		log      logger.Logger
	...
	tests := []struct {
		...
		fields  fields
		args    args
		want    *model.DiscordGuildStat
		wantErr bool
	}{
		// TODO: Add test case.
		{
			name: "guild found successfully",
			fields: fields{
				repo: r,
			},
			args: args{
	...
	disGuStat.EXPECT().GetByGuildID("863278424433229854").Return(&guildStatValid, nil).AnyTimes()
	disGuStat.EXPECT().GetByGuildID("abc").Return(nil, errors.New("error: invalid guild id")).AnyTimes()
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			e := &Entity{
				repo:     tt.fields.repo,
			...
			got, err := e.GetByGuildID(tt.args.guildID)
			if (err != nil) != tt.wantErr {
				t.Errorf("Entity.GetByGuildID() error = %v, wantErr %v", err, tt.wantErr)
				return
			}
			if !reflect.DeepEqual(got, tt.want) {
				t.Errorf("Entity.GetByGuildID() = %v, want %v", got, tt.want)
			}
		})
	}
}
